...
const Admin = () => {
    ...
    try {
      ...
      await supabase
        ...
        .insert({
          ...
          details: `Approved user registration for ${userEmail}`
        });

      toast({
        title: "User approved",
        description: `${userEmail} has been approved and can now access the system.`,
      });
    ...
    try {
      ...
      await supabase
        ...
        .insert({
          ...
          target_user_id: userId,
          details: `Rejected user registration for ${userEmail}${reason ? `: ${reason}` : ''}`
        });

      toast({
        title: "User rejected",
        description: `${userEmail} registration has been rejected.`,
      });
  ...
  const createUserAccount = async (formData: CreateUserForm) => {
    ...
    try {
      ...
      if (data.user) {
        // Update profile with admin-created status
        ...
        await supabase
          ...
          .insert({
            ...
            details: `Created user account for ${formData.email} (${formData.traderName})`
          });
      }
  ...
  const pauseUser = async (userId: string, userEmail: string) => {
    try {
      const { error } = await supabase
        .from('profiles')
      ...
      await supabase
        ...
        .insert({
          ...
          target_user_id: userId,
          details: `Paused user account: ${userEmail}`
        });

      toast({
        title: "User paused",
        description: `${userEmail} account has been paused.`,
      });
  ...
  const resumeUser = async (userId: string, userEmail: string) => {
    try {
      const { error } = await supabase
        .from('profiles')
      ...
      await supabase
        ...
        .insert({
          ...
          target_user_id: userId,
          details: `Resumed user account: ${userEmail}`
        });

      toast({
        title: "User resumed",
        description: `${userEmail} account has been resumed.`,
      });
