  ...
  const loadUserProfile = async (userId: string) => {
    try {
      const {
        data,
        error
      } = await (supabase as any).from('profiles').select('*').eq('user_id', userId).single();
      ...
      await Promise.all([loadAllUsers(), loadUserStats(), loadNotifications(), loadPendingUsers(), loadRejectedUsers(), loadActiveSessions()]);
    } catch (error: any) {
  ...
  const loadAllUsers = async () => {
    try {
      const {
        data,
        error
      } = await (supabase as any).from('profiles').select('*').order('created_at', {
        ascending: false
      });
      if (error) throw error;
  ...
  const loadUserStats = async () => {
    try {
      ...
      const {
        data: allUsers,
        error: usersError
      } = await supabase.from('profiles').select('user_id, trader_name, created_at');
  ...
  const loadNotifications = async () => {
    try {
      ...
      const {
        data,
        error
      } = await (supabase as any).from('admin_notifications').select('*').order('created_at', {
        ascending: false
      }).limit(10);
      if (!error && data) {
  ...
  return <div className="min-h-screen bg-background">
              ...
              <DropdownMenu open={notificationDropdownOpen} onOpenChange={setNotificationDropdownOpen}>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" className="relative p-2">
                    ...
                    {unreadNotifications > 0 && <Badge className="absolute -top-1 -right-1 h-5 w-5 p-0 flex items-center justify-center text-xs">
                        {unreadNotifications}
                      </Badge>}
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent className="w-80 max-h-96 overflow-y-auto">
                  {notifications.length === 0 ? <div className="p-4 text-center text-muted-foreground">
                      No notifications
                    </div> : notifications.slice(0, 5).map(notification => <div key={notification.id} className={`p-3 border-b last:border-b-0 cursor-pointer transition-colors hover:bg-muted/50 ${!notification.is_read ? 'bg-primary/5 border-l-4 border-l-primary' : ''}`} onClick={() => handleNotificationClick(notification.id)}>
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              {(notification as any).processed && <span className={`text-xs px-2 py-1 rounded ${(notification as any).action === 'approved' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                                  {(notification as any).action === 'approved' ? 'Approved' : 'Rejected'}
                                </span>}
                            </div>
                            {notification.type === 'registration' && notification.user_id && !(notification as any).processed && <div className="flex gap-1 mt-2" onClick={e => e.stopPropagation()}>
                                <Button size="sm" variant="outline" onClick={() => approveFromNotification(notification.user_id!, notification.id)} className="text-xs h-6 px-2 text-green-600 hover:text-green-700">
                                  Approve
                                </Button>
                                <Button size="sm" variant="outline" onClick={() => rejectFromNotification(notification.user_id!, notification.id)} className="text-xs h-6 px-2 text-red-600 hover:text-red-700">
                                  Reject
                                </Button>
                              </div>}
                          </div>
      ...
      <TabsContent value="registrations" className="space-y-6">
        <Card>
          ...
          <CardContent>
            {pendingUsers.length === 0 ? <div className="text-center py-8">
      ...
      <TabsContent value="rejected" className="space-y-6">
        <Card>
          ...
          <CardContent>
            {rejectedUsers.length === 0 ? <div className="text-center py-8">
                ...
                <TableBody>
                  {rejectedUsers.map(user => <TableRow key={user.id}>
                      <TableCell className="font-medium">{user.trader_name}</TableCell>
                      <TableCell>{user.full_name || '-'}</TableCell>
                      <TableCell>{new Date(user.created_at).toLocaleDateString()}</TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <Button variant="outline" size="sm" onClick={() => reApproveUser(user.user_id)} className="text-green-600 hover:text-green-700">
                          <CheckCircle className="h-4 w-4 mr-1" />
                          Re-approve
                        </Button>
                      </TableCell>
                ...
                {notifications.map(notification => <div key={notification.id} className={`p-4 border rounded-lg cursor-pointer transition-colors hover:bg-muted/50 ${notification.is_read ? 'bg-muted/50' : 'bg-card border-primary/20'}`} onClick={() => handleNotificationClick(notification.id)}>
                    <div className="flex items-start justify-between">
                      ...
                      <div className="flex gap-2">
                        ...
                        {(notification as any).processed && <Badge variant={(notification as any).action === 'approved' ? 'default' : 'destructive'}>
                            {(notification as any).action === 'approved' ? 'Approved' : 'Rejected'}
                          </Badge>}
                      </div>
                    </div>
