...
serve(async (req) => {
  ...
  try {
    ...
    try {
      // If we have recent cached data, return it to reduce API calls during rate limiting
      const recentCachedEvents = cachedEvents.filter(e => {
        const eventTime = new Date(e.event_time || e.created_at);
        const now = new Date();
        const timeDiff = now.getTime() - eventTime.getTime();
        return timeDiff < (30 * 60 * 1000); // Events within last 30 minutes
      });

      if (recentCachedEvents.length > 0) {
        console.log(`Using ${recentCachedEvents.length} recent cached events to avoid rate limiting`);
        filtered = recentCachedEvents.map(e => ({
          title: e.title,
          currency: e.currency,
        ...
        // Attempt to fetch fresh data with improved retry logic
        let xmlResp;
        let retries = 3;
        ...
        while (retries > 0) {
          try {
            ...
            if (xmlResp.status === 429) {
              console.warn(`Rate limited (429), waiting ${waitTime}ms before retry... ${retries} retries left`);
            ...
            throw new Error(`Feed error: ${xmlResp.status} ${xmlResp.statusText}`);
          } catch (fetchError) {
            retries--;
            if (retries === 0) throw fetchError;
            console.warn(`Fetch failed, retrying in ${waitTime}ms... ${retries} retries left`, fetchError);
            await new Promise(resolve => setTimeout(resolve, waitTime));
            waitTime = Math.min(waitTime * 1.5, 10000); // Cap at 10 seconds
          }
        }
