...
-- Fix the forex events monitor function to use net.http_post (no cross-database references)
...
-- Create BEFORE UPDATE triggers to keep updated_at in sync across tables
DO $$ BEGIN
...
-- Ensure public bucket exists for uploads
INSERT INTO storage.buckets (id, name, public)
VALUES ('99solutiontech', '99solutiontech', true)
ON CONFLICT (id) DO NOTHING;

-- Storage policies for the 99solutiontech bucket
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'storage' AND tablename = 'objects' AND polname = 'Public can read 99solutiontech'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'storage' AND tablename = 'objects' AND polname = 'Users can upload to 99solutiontech in own folder'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'storage' AND tablename = 'objects' AND polname = 'Users can update own 99solutiontech objects'
  ) THEN
